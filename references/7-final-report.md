...
阶段 7 汇总所有已执行阶段的已确认配置、过程、结果、决策和图表，生成最终建模报告。阶段 7 不重新训练模型、不重新计算核心指标、不修改上游决策，也不包含上线与持续监控。
...
| 已执行阶段的 `00-output-list.xlsx` 至 `06-output-list.xlsx` | 各阶段输出索引与审阅汇总 |
| `05_evaluation_decisions.csv` | 模型评估结论及回退记录 |
| `06_scoring_decisions.csv`、`06_scoring_parameters.json` | 仅当 `enable_stage6_scoring=true` 时必需 |

### 路径相关输入

- LR：阶段 3 最终分箱和阶段 4 LR 训练过程；启用阶段 6 时额外汇总标准评分卡明细与一致性验证。
- XGB/LGB：阶段 4 调优过程和阶段 5 评估结果；启用阶段 6 时额外汇总校准前后比较。
- 存在灰样本且启用阶段 6 时：阶段 1 灰样本说明与阶段 6 灰样本评分汇总。
...
1. 最终结论与建议：适用范围、已知限制、阶段 5 评估决策、启用阶段 6 时的评分决策及人工确认记录。
2. 项目背景、Y_label定义、样本时间范围和模型类型。
3. 数据准备、Train/Test/OOT划分和样本统计。
4. EDA核心结论与数据质量决策。
5. 特征筛选、分箱、人工调整和最终特征。
6. 模型训练过程：
   - LR：逐步回归、AIC/BIC变化、诊断、最终系数与截距。
   - XGB/LGB：调优过程、Trial变化、最佳参数和最终选择依据。
7. 模型评估：KS、AUC、Gini、Lift、PSI、周期PSI、校准度及回退处理。
8. 启用阶段 6 时，展示评分结果：概率校准、评分参数、分数分布、风险等级和灰样本结果。
9. LR且启用阶段 6 时，展示标准评分卡：逐变量逐分箱分值、基础分和一致性验证。
10. 文件交付清单与生成时间。

“最终结论与建议”必须位于报告标题之后、其他正文之前，只引用 `05_evaluation_decisions.csv` 和启用阶段 6 时的 `06_scoring_decisions.csv` 中已确认的记录。阶段 7 不根据指标自行推断新结论；决策表存在 `pending`、无法读取或没有已确认记录时，必须写入 `07_report_issues.csv` 并暂停交付。
...
2. 阶段 0-5 均已完成；仅当 `enable_stage6_scoring=true` 时要求阶段 6 完成。所有已执行阶段不存在未处理的 `pending`。
