...
仅当 `enable_stage6_scoring=true` 时，模型进入阶段 6 后执行以下处理规则：
...
| 区分度与排序可接受，但概率偏差明显 | 校准问题 | Reliability、Brier、Log Loss、校准截距/斜率 | 启用阶段 6 时执行校准；未启用时记录限制并进入最终报告 |

L1/L2 是处理过拟合的候选手段，不是看到数据集差异后的默认动作。若主要原因是数据漂移、切分异常或特征失效，应回退至对应上游阶段处理。

### 5.5.2 回退与重新验证规则

1. 评估结论为过拟合、欠拟合、数据漂移、切分异常或排序问题时，不得进入后续阶段。
...
| Brier、Log Loss、Reliability、校准截距/斜率 | `poor_calibration` | 启用阶段 6 时：LR接受原始概率或校准，XGB/LGB必须校准；未启用阶段 6 时记录评估结论 |

任一核心决策仍为 `pending` 时，不得进入下一阶段。
...
7. 如存在回退决策，阶段 5 状态必须标记为未通过；仅在回退处理并重新评估通过后，才允许进入后续阶段。

阶段 5 验收通过后：

- `enable_stage6_scoring=true`：进入阶段 6。
- `enable_stage6_scoring=false`：跳过阶段 6，直接进入阶段 7 最终报告。
