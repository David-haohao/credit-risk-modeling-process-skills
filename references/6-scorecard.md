...
- 如未来可以获得 Train 的 OOF 预测或独立校准集，应优先替换训练内预测。
...
当前不固定采用 `[300, 900]` 或其他范围。是否设置分数上下限、上下限取值以及截断后处理方式，必须由用户确认。
...
输出 `06_scorecard_validation.csv`，至少包含数据集、样本数、最大绝对误差、平均绝对误差、差异分布和是否通过。整数分取整后的允许误差规则由用户确认；未通过时不得完成阶段 6。
